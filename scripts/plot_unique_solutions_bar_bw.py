#!/usr/bin/env python3
"""Black & white faceted histogram for unique solution counts by num_samples.

Input CSV columns required:
  outload_key, num_samples, unique_solution_count

Chart:
    x-axis: unique_solution_count (integer levels)
    y-axis: frequency (count of outload_key rows producing that number of unique solutions)
...
Run with no arguments to use hardcoded defaults:
  input  = data/results/phase1_unique_solution_counts.csv
  output = data/results/unique_solutions_lollipop_faceted_bw.pdf
...
DEFAULT_INPUT = Path("data/results/phase1_unique_solution_counts.csv")
DEFAULT_OUTPUT = Path("data/results/unique_solutions_lollipop_faceted_bw.pdf")
DEFAULT_PGF = None
...
def parse_args():
    p = argparse.ArgumentParser(description="Lollipop frequency chart for unique solution counts")
    ...
    p.add_argument('--min-count', type=int, help='Minimum x (unique_solution_count) to display')
    p.add_argument('--max-count', type=int, help='Maximum x (unique_solution_count) to display')
...
def load_data(path: Path) -> pd.DataFrame:
    df = pd.read_csv(path)
    needed = {"outload_key", "num_samples", "unique_solution_count"}
...
def compute_freq(df: pd.DataFrame, samples: list[int] | None):
    ...
    freq = (df.groupby(['num_samples', 'unique_solution_count'])
              .size()
...
def plot_faceted(freq: pd.DataFrame, output: Path, pgf: Path | None, usetex: bool, samples_order: list[int], min_count: int | None, max_count: int | None):
    ...
    overall_min = freq['unique_solution_count'].min() if min_count is None else min_count
    overall_max = freq['unique_solution_count'].max() if max_count is None else max_count
    ymax = freq['count'].max()
    ...
    for ax, s in zip(axes, samples_order):
        sub = (freq[freq.num_samples==s]
                 .sort_values('unique_solution_count'))
        # Ensure full x range (insert missing counts with zero freq)
        existing = set(sub['unique_solution_count'])
        filler_rows = []
        for c in range(overall_min, overall_max+1):
            if c not in existing:
                filler_rows.append({'num_samples': s, 'unique_solution_count': c, 'count': 0})
        if filler_rows:
            sub = pd.concat([sub, pd.DataFrame(filler_rows)], ignore_index=True).sort_values('unique_solution_count')
        x = sub['unique_solution_count'].values
        y = sub['count'].values
        ...
        ax.set_xlabel('Unique Solutions')
...
def main():
    ...
    print(f"Saved unique solutions lollipop chart to {args.output}" + (f" and {args.pgf}" if args.pgf else ""))
