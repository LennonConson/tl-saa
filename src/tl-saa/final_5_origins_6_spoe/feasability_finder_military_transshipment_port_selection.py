...
def scenario_creator(scenario_name, divisions_per_day=3, replication=0, max_days=17):
    ...
    model = pysp_instance_creation_callback(scenario_name, divisions_per_day, replication, max_days)
    sputils.attach_root_node(model, model.FirstStageCost, [model.y_open])    
    return model

def pysp_instance_creation_callback(scenario_name, divisions_per_day, replication, max_days):
    ...
    data_dir = os.path.join(os.path.dirname(__file__), '../../../data')
    percentile_delays_path = os.path.abspath(os.path.join(data_dir, 'spoe_percentile_delays_i5_j6.pkl'))
    ...
    outload_path = os.path.abspath(os.path.join(data_dir, 'outload_i5_samp100.pkl'))
    with open(outload_path, 'rb') as f:
        all_outload_requirements = pickle.load(f)
    print(all_outload_requirements)
    print(scenario_name)
    outload_values = all_outload_requirements[scenario_name]
    l_outload = 14000
    u_outload = 21000
    outload_requirements = {i+1: l_outload + outload_values[i] * (u_outload - l_outload) for i in range(num_I)}
