...
def scenario_creator(scenario_name, divisions_per_day=3, replication=0, day_add=0):
    ...
    model = pysp_instance_creation_callback(scenario_name, divisions_per_day, replication, day_add)
    sputils.attach_root_node(model, model.FirstStageCost, [model.y_open])    

    return model

def pysp_instance_creation_callback(scenario_name, divisions_per_day, replication, day_add=0):
    ...
    max_days = feasibility_dict.get((scenario_name, replication, divisions_per_day), 0)
    max_days += day_add
    print(f"Max days for scenario {scenario_name} with replication {replication} and divisions_per_day {divisions_per_day}: {max_days}")
    # max_days = 17
    ...
    ship_layberth = {1:1, 2:2, 3:3, 4:6, # Bob Hope
